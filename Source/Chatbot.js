        ...
        let userDocRef, apiKey = '', memory = [], chatHistory = [];
        ...
        function render(text, role) {
            ...
            function _renderCode(code, lang) {
                ...
                const lbl = document.createElement('span'); lbl.className = 'code-block-lang';
        ...
        auth.onAuthStateChanged(async user => {
            ...
            if (!snap.exists) {
                await userDocRef.set({ apiKey: null, memory: [], chatHistory: [] });
            }
        ...
        saveApiKeyBtn.onclick = async () => {
            ...
            apiKey = key;
            await userDocRef.update({ apiKey });
            apiKeyModal.classList.add('hidden');
            chatApp.style.display = 'flex';
            saveApiKeyBtn.disabled = false;
            saveApiKeyBtn.textContent = 'Validate & Save';
        ...
        sendBtn.onclick = async () => {
            ...
            await userDocRef.update({ chatHistory });
            ...
            if (unique.length) {
                memory.push(...unique);
                await userDocRef.update({ memory });
            }
        };
